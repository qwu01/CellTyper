...
def generate_split_idx(cell_path: Path, split_idx_path="Data/split_idx.json", reproducible_random_state=42):
    ...
    """
    # write a sampler to make a meaningful split of training/test dataset.
    # Make sure that the CellType labels are relatively balanced.
    # Make sure that the Method annotations are relatively balanced.
    ...
    # For now, consider [Method] and [CellType] columns, split based on these two columns. 
    # Use n_splits=1 to split one training set and one tst set.
    # Use reproducible_random_state to make sure it is reproducible.
